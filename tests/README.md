...
- **T1** version/help 输出
- **T2** 脏状态（5054/5055 残留、明文上游、双实例、notrack_dns、DNSMASQ HIJACK）下 `check` 应报 FAIL 且 rc=1
- **T3** `install` 从脏状态修复：逐条校验 uci 调用序列（noresolv/dns_redirect/清理/单实例化/force_dns 53+853）、canary 保留、自动备份、daed 片段生成（endpoint 规则、premium/proxy 特例、fallback）、服务重启顺序
